...
impl MarkerParser {
    fn parse_xml<T: Peek + Next>(tag_name: &str, stack: &mut T) -> Result<Marker, XmlParseError> {
        ...
        let chars = match characters(stack) {
            Ok(obj) => obj,
            Err(e) => "".to_string()
        };
...
impl<P, D> S3Client<P, D> where P: ProvideAwsCredentials, D: DispatchSignedRequest {
    ...
    pub fn list_objects(&self, input: &ListObjectsRequest) -> Result<ListObjectsOutput, S3Error> {
        ...
        //println!("S3 returned '{:?}'", String::from_utf8(result.body.clone()));
        let string = String::from_utf8(result.body).unwrap();
        let mut reader = EventReader::from_str(string.as_str());
    ...
    pub fn get_object(&self, input: &GetObjectRequest) -> Result<GetObjectOutput, S3Error> {
        ...
        match status {
            ...
            _ => {
                ...
                //println!("Error response body: {:?}", String::from_utf8(result.body.clone()));
                Err(S3Error::new(format!("error in get_object, status {}", status)))
            }
        }
    }
...
fn extract_s3_redirect_location(response: HttpResponse) -> Result<String, S3Error> {
    ...
    let string = String::from_utf8(response.body).unwrap();
    let mut reader = EventReader::from_str(string.as_str());
    let mut stack = XmlResponse::new(reader.events().peekable());
    stack.next(); // xml start tag
